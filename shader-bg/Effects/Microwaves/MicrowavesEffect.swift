...
class MicrowavesEffect: VisualEffect {
  var name: String = "Microwaves"
  var displayName: String = "Microwaves"

  private var renderer: MicrowavesRenderer?
  private var commandQueue: MTLCommandQueue?
  private var lastUpdateTime: Double = 0

  func setup(device: MTLDevice, size: CGSize) {
    commandQueue = device.makeCommandQueue()
    renderer = MicrowavesRenderer(device: device, size: size)

    print("✅ MicrowavesEffect 初始化完成")
  }
