// Forked from https://www.shadertoy.com/view/WtjfRG
...
static float micro_w(float x, float p, float time) {
  ...
  return (sin(x * 0.25 + t) * 5.0 + sin(x * 4.5 + t * 3.0) * 0.2 +
          sin(x + t * 3.0) * 2.3 + sin(x * 0.8 + t * 1.1) * 2.5) *
         0.275;
}

static float4 microwaves_effect(float2 fragCoord, float time,
                                float2 resolution) {
  ...
  for (float i = -5.0; i < 5.0; i++) {
    ...
    float3 palette =
        micro_palette(sin((n - i) * 0.15), float3(0.5), float3(0.5),
                      float3(0.270), float3(0.0, 0.05, 0.15));
...
kernel void microwavesCompute(texture2d<float, access::write> output
                              [[texture(0)]],
