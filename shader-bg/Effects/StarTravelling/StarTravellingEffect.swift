...
final class StarTravellingEffect: VisualEffect {
  let name: String = "star_travelling"
  let displayName: String = "Star Travelling"

  var preferredFramesPerSecond: Int { 20 }
  var occludedFramesPerSecond: Int { 5 }

  private var renderer: StarTravellingRenderer?
  private var startTime: CFTimeInterval?

  func setup(device: MTLDevice, size: CGSize) {
    renderer = StarTravellingRenderer(device: device, size: size)
    startTime = nil
  }
