...
class KorotkoeRenderer {
  ...
  var updateInterval: Double = 1.0 / 15.0  // 15 FPS - 降低帧率减少GPU开销
  ...
  private func setupPipeline() {
    ...
    let vertexFunction = library.makeFunction(name: "korotkoeVertex")
    let fragmentFunction = library.makeFunction(name: "korotkoeFragment")
  ...
  private func setupBuffers() {
    ...
    paramsBuffer = device.makeBuffer(
      length: MemoryLayout<KorotkoeParams>.stride,
  ...
  func updateParticles(currentTime: CFTimeInterval) {
    ...
    // 更新时间 - 减慢到原速度的 1/80 (1/10 * 1/8)
    time += 0.0005  // 15fps * 0.0005 = 每秒增加约 0.0075 (原速度的 1/80)
  }
  ...
  func draw(in view: MTKView) {
    ...
    // 每次绘制更新时间 - 减慢到原速度的 1/80 (1/10 * 1/8)
    time += 0.0005  // 15fps * 0.0005 = 每秒增加约 0.0075 (原速度的 1/80)
  ...
  private func updateParams(viewportSize: CGSize) {
    ...
    paramsBuffer.contents().copyMemory(
      from: &params, byteCount: MemoryLayout<KorotkoeParams>.stride)
  }
}
