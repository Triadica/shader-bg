...
class InfiniteRingEffect: VisualEffect {
  var name: String = "InfiniteRing"
  var displayName: String = "Infinite Ring"

  private var renderer: InfiniteRingRenderer?
  private var commandQueue: MTLCommandQueue?
  private var lastUpdateTime: Double = 0

  func setup(device: MTLDevice, size: CGSize) {
    print("🔵 [InfiniteRing] 开始初始化...")
    commandQueue = device.makeCommandQueue()
    renderer = InfiniteRingRenderer(device: device, size: size)
    print("✅ [InfiniteRing] 初始化完成，准备渲染")
  }
  ...
  func draw(in view: MTKView) {
    guard let renderer = renderer,
      ...
      print("⚠️ [InfiniteRing] draw 失败：renderer/commandQueue/commandBuffer 为 nil")
      return
    }
