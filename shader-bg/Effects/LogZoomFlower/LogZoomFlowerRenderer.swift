...
class LogZoomFlowerRenderer {
  ...
  private func setupPipeline() {
    ...
    guard let function = library.makeFunction(name: "logZoomFlowerCompute") else {
      fatalError("无法找到 logZoomFlowerCompute 函数")
    }
  ...
  func draw(commandBuffer: MTLCommandBuffer, view: MTKView) {
    ...
    computeEncoder.setBytes(&data, length: MemoryLayout<LogZoomFlowerData>.stride, index: 0)
