...
class CityOfKaliRenderer {
  ...
  var updateInterval: Double = 1.0 / 30.0  // 30 FPS 提升流畅度
  ...
  private func setupPipeline() {
    ...
    let vertexFunction = library.makeFunction(name: "cityOfKaliVertex")
    let fragmentFunction = library.makeFunction(name: "cityOfKaliFragment")
  ...
  private func setupBuffers() {
    ...
    paramsBuffer = device.makeBuffer(
      length: MemoryLayout<CityOfKaliParams>.stride,
  ...
  func updateParticles(currentTime: CFTimeInterval) {
    ...
    // 更新时间（每秒移动距离 = 30fps * 0.001 / 30 = 0.001）
    time += Float(updateInterval) * (0.001 / Float(updateInterval))
  }
  ...
  func draw(in view: MTKView) {
    ...
    // 每次绘制更新时间（每秒移动距离 = 30fps * 0.001 / 30 = 0.001）
    time += Float(updateInterval) * (0.001 / Float(updateInterval))
  ...
  private func updateParams(viewportSize: CGSize) {
    ...
    paramsBuffer.contents().copyMemory(
      from: &params, byteCount: MemoryLayout<CityOfKaliParams>.stride)
  }
}
