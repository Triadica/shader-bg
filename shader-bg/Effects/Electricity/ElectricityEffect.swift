...
class ElectricityEffect: VisualEffect {
  var name: String = "Electricity"
  var displayName: String = "Electricity"

  private var renderer: ElectricityRenderer?
  private var commandQueue: MTLCommandQueue?
  private var lastUpdateTime: Double = 0

  func setup(device: MTLDevice, size: CGSize) {
    commandQueue = device.makeCommandQueue()
    renderer = ElectricityRenderer(device: device, size: size)

    print("✅ ElectricityEffect 初始化完成")
  }
