...
// 绘制小鱼（极简版 - 椭圆 + 三角形尾巴）
float drawFish(float2 uv, float2 pos, float2 vel, float size, float phase,
               float aspect) {
  // 快速距离检查 - 太远直接返回0
  float quickDist = abs(uv.x - pos.x) + abs(uv.y - pos.y);
  if (quickDist > size * 5.0)
    return 0.0;
  ...
  float body =
      (1.0 - smoothstep(currentWidth * 0.7, currentWidth, acrossAbs)) * inBody;
...
kernel void swimmingFishCompute(texture2d<float, access::write> output
                                [[texture(0)]],
  ...
  for (int i = 0; i < data.fishCount && i < 20; i++) {
    Fish fish = fishes[i];
    float fishAlpha = drawFish(uv, fish.position, fish.velocity, fish.size,
                               fish.phase, aspect);
