...
class HyperbolicRingsRenderer {
  ...
  private func setupPipeline() {
    ...
    let vertexFunction = library.makeFunction(name: "hyperbolicRingsVertex")
    let fragmentFunction = library.makeFunction(name: "hyperbolicRingsFragment")
  ...
  private func setupBuffers() {
    ...
    paramsBuffer = device.makeBuffer(
      length: MemoryLayout<HyperbolicRingsParams>.stride,
  ...
  func updateParticles(currentTime: CFTimeInterval) {
    ...
    // 更新时间 - 减慢到原速度的 1/24
    time += 0.000416  // 30fps * 0.000416 ≈ 每秒增加约 0.0125 (原速度的 1/24)
  }
  ...
  private func updateParams(viewportSize: CGSize) {
    ...
    var params = HyperbolicRingsParams(
      time: time,
      resolution: SIMD2<Float>(Float(viewportSize.width), Float(viewportSize.height)),
      mouse: SIMD2<Float>(0, 0),  // 自动模式
      padding: 0
    )

    paramsBuffer.contents().copyMemory(
      from: &params, byteCount: MemoryLayout<HyperbolicRingsParams>.stride)
  }
}
