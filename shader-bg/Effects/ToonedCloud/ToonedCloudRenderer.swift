...
class ToonedCloudRenderer {
  ...
  private func setupPipeline() {
    ...
    let vertexFunction = library.makeFunction(name: "toonedCloudVertex")
    let fragmentFunction = library.makeFunction(name: "toonedCloudFragment")
  ...
  private func setupBuffers() {
    ...
    paramsBuffer = device.makeBuffer(
      length: MemoryLayout<ToonedCloudParams>.stride,
  ...
  func draw(in view: MTKView) {
    ...
    renderPassDescriptor.colorAttachments[0].clearColor = MTLClearColor(
      red: 0.0, green: 0.4, blue: 0.58, alpha: 1.0)
  ...
  private func updateParams(viewportSize: CGSize) {
    ...
    paramsBuffer.contents().copyMemory(
      from: &params, byteCount: MemoryLayout<ToonedCloudParams>.stride)
  }
}
