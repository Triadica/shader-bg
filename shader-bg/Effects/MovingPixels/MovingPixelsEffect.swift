...
class MovingPixelsEffect: VisualEffect {
  var name: String = "MovingPixels"
  var displayName: String = "Moving Pixels"

  private var renderer: MovingPixelsRenderer?
  private var commandQueue: MTLCommandQueue?
  private var lastUpdateTime: Double = 0

  func setup(device: MTLDevice, size: CGSize) {
    commandQueue = device.makeCommandQueue()
    renderer = MovingPixelsRenderer(device: device, size: size)

    print("✅ MovingPixelsEffect 初始化完成")
  }
