...
class StainedLightsRenderer {
  ...
  private func setupPipeline() {
    ...
    let vertexFunction = library.makeFunction(name: "stainedLightsVertex")
    let fragmentFunction = library.makeFunction(name: "stainedLightsFragment")
  ...
  private func setupBuffers() {
    ...
    paramsBuffer = device.makeBuffer(
      length: MemoryLayout<StainedLightsParams>.stride,
  ...
  func updateParticles(currentTime: CFTimeInterval) {
    ...
    // 更新时间（每帧增加固定值，让动画可见）
    time += 0.001  // 每帧增加 0.001，每秒增加约 0.03
  }
  ...
  private func updateParams(viewportSize: CGSize) {
    ...
    var params = StainedLightsParams(
      time: time,
    ...
    paramsBuffer.contents().copyMemory(
      from: &params, byteCount: MemoryLayout<StainedLightsParams>.stride)
  }
}
