...
class GeodeBGRenderer {
  ...
  private func setupPipeline() {
    ...
    let vertexFunction = library.makeFunction(name: "geodeBGVertex")
    let fragmentFunction = library.makeFunction(name: "geodeBGFragment")
  ...
  private func setupBuffers() {
    ...
    paramsBuffer = device.makeBuffer(
      length: MemoryLayout<GeodeBGParams>.stride,
  ...
  func updateParticles(currentTime: CFTimeInterval) {
    ...
    // 更新时间 - 加快到原速度的10倍左右
    time += 0.016  // 30fps * 0.016 = 每秒增加约 0.48
  }
  ...
  private func updateParams(viewportSize: CGSize) {
    ...
    paramsBuffer.contents().copyMemory(
      from: &params, byteCount: MemoryLayout<GeodeBGParams>.stride)
  }
}
