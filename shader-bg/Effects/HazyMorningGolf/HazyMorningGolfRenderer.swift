...
class HazyMorningGolfRenderer {
  ...
  private func setupPipeline() {
    ...
    let vertexFunction = library.makeFunction(name: "hazyMorningGolfVertex")
    let fragmentFunction = library.makeFunction(name: "hazyMorningGolfFragment")
  ...
  private func setupBuffers() {
    ...
    paramsBuffer = device.makeBuffer(
      length: MemoryLayout<HazyMorningGolfParams>.stride,
  ...
  func updateParticles(currentTime: CFTimeInterval) {
    ...
    // 更新时间 - 减慢到原速度的 1/16
    time += 0.001  // 30fps * 0.001 = 每秒增加约 0.03 (原速度的 1/16)
  }
  ...
  private func updateParams(viewportSize: CGSize) {
    ...
    var params = HazyMorningGolfParams(
      time: time,
    ...
    paramsBuffer.contents().copyMemory(
      from: &params, byteCount: MemoryLayout<HazyMorningGolfParams>.stride)
  }
}
