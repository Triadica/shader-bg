...
final class SonataEffect: VisualEffect {
  let name: String = "sonata"
  let displayName: String = "Sonata"

  var preferredFramesPerSecond: Int { 20 }
  var occludedFramesPerSecond: Int { 5 }

  private var renderer: SonataRenderer?
  private var startTime: CFTimeInterval?

  func setup(device: MTLDevice, size: CGSize) {
    renderer = SonataRenderer(device: device, size: size)
    startTime = nil
  }
