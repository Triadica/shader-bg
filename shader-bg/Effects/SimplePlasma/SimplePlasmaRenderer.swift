...
class SimplePlasmaRenderer {
  ...
  private func setupPipeline() {
    ...
    let vertexFunction = library.makeFunction(name: "simplePlasmaVertex")
    let fragmentFunction = library.makeFunction(name: "simplePlasmaFragment")
  ...
  private func setupBuffers() {
    ...
    paramsBuffer = device.makeBuffer(
      length: MemoryLayout<SimplePlasmaParams>.stride,
  ...
  func updateParticles(currentTime: CFTimeInterval) {
    ...
    // 更新时间（减慢到原始速度的 1/32，即当前的 1/4）
    time += 0.0003125  // 30fps * 0.0003125 = 每秒增加约 0.009375（原速度的 1/32）
  }
  ...
  func draw(in view: MTKView) {
    ...
    // 每次绘制更新时间（减慢到原始速度的 1/32，即当前的 1/4）
    time += 0.0003125  // 30fps * 0.0003125 = 每秒增加约 0.009375（原速度的 1/32）
  ...
  private func updateParams(viewportSize: CGSize) {
    ...
    paramsBuffer.contents().copyMemory(
      from: &params, byteCount: MemoryLayout<SimplePlasmaParams>.stride)
  }
}
