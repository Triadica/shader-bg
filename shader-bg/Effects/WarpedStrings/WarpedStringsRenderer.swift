...
class WarpedStringsRenderer {
  ...
  var updateInterval: Double = 1.0 / 20.0  // 20 FPS (降低帧率以减少 GPU 负载)
  ...
  private func setupPipeline() {
    ...
    let vertexFunction = library.makeFunction(name: "warpedStringsVertex")
    let fragmentFunction = library.makeFunction(name: "warpedStringsFragment")
  ...
  private func setupBuffers() {
    ...
    paramsBuffer = device.makeBuffer(
      length: MemoryLayout<WarpedStringsParams>.stride,
  ...
  func updateParticles(currentTime: CFTimeInterval) {
    ...
    // 更新时间（减慢到原始速度的 1/8）
    time += 0.001875  // 20fps * 0.001875 = 每秒增加约 0.0375（原速度的 1/8）
  }
  ...
  private func updateParams(viewportSize: CGSize) {
    ...
    var params = WarpedStringsParams(
      time: time,
    ...
    paramsBuffer.contents().copyMemory(
      from: &params, byteCount: MemoryLayout<WarpedStringsParams>.stride)
  }
}
