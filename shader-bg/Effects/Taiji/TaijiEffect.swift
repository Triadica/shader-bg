...
class TaijiEffect: VisualEffect {
  var name: String = "Taiji"
  var displayName: String = "Taiji"

  private var renderer: TaijiRenderer?
  private var commandQueue: MTLCommandQueue?
  private var lastUpdateTime: Double = 0

  func setup(device: MTLDevice, size: CGSize) {
    commandQueue = device.makeCommandQueue()
    renderer = TaijiRenderer(device: device, size: size)

    print("✅ TaijiEffect 初始化完成")
  }
