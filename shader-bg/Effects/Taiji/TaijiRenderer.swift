...
class TaijiRenderer {
  ...
  private func setupPipeline() {
    ...
    guard let function = library.makeFunction(name: "taijiCompute") else {
      fatalError("无法找到 taijiCompute 函数")
    }
  ...
  func draw(commandBuffer: MTLCommandBuffer, view: MTKView) {
    ...
    computeEncoder.setBytes(&data, length: MemoryLayout<TaijiData>.stride, index: 0)
