...
class ButterflyAIRenderer {
  ...
  private func setupPipeline() {
    ...
    let vertexFunction = library.makeFunction(name: "butterflyAIVertex")
    let fragmentFunction = library.makeFunction(name: "butterflyAIFragment")
  ...
  private func setupBuffers() {
    ...
    paramsBuffer = device.makeBuffer(
      length: MemoryLayout<ButterflyAIParams>.stride,
  ...
  func updateParticles(currentTime: CFTimeInterval) {
    ...
    // 更新时间 - 减慢到原速度的 1/4
    time += 0.004  // 30fps * 0.004 = 每秒增加约 0.12 (原速度的 1/4)
  }
  ...
  private func updateParams(viewportSize: CGSize) {
    ...
    var params = ButterflyAIParams(
      time: time,
      resolution: SIMD2<Float>(Float(viewportSize.width), Float(viewportSize.height)),
      mouse: mousePosition * SIMD2<Float>(Float(viewportSize.width), Float(viewportSize.height)),
      padding: 0
    )

    paramsBuffer.contents().copyMemory(
      from: &params, byteCount: MemoryLayout<ButterflyAIParams>.stride)
  }
}
