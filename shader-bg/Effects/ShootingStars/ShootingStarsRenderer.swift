...
class ShootingStarsRenderer {
  ...
  private func setupPipeline() {
    ...
    let vertexFunction = library.makeFunction(name: "shootingStarsVertex")
    let fragmentFunction = library.makeFunction(name: "shootingStarsFragment")
  ...
  private func setupBuffers() {
    ...
    paramsBuffer = device.makeBuffer(
      length: MemoryLayout<ShootingStarsParams>.stride,
  ...
  private func updateParams(viewportSize: CGSize) {
    ...
    var params = ShootingStarsParams(
      time: time,
    ...
    paramsBuffer.contents().copyMemory(
      from: &params, byteCount: MemoryLayout<ShootingStarsParams>.stride)
  }
}
