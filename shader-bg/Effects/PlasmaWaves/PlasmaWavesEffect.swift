...
class PlasmaWavesEffect: VisualEffect {
  var name: String = "PlasmaWaves"
  var displayName: String = "Plasma Waves"

  private var renderer: PlasmaWavesRenderer?
  private var commandQueue: MTLCommandQueue?
  private var lastUpdateTime: Double = 0

  func setup(device: MTLDevice, size: CGSize) {
    commandQueue = device.makeCommandQueue()
    renderer = PlasmaWavesRenderer(device: device, size: size)

    print("✅ PlasmaWavesEffect 初始化完成")
  }
