...
class JuliaSetRenderer {
  ...
  var updateInterval: Double = 1.0 / 15.0  // 降低帧率到 15 FPS
  ...
  private func setupPipeline() {
    ...
    guard let function = library.makeFunction(name: "juliaSetCompute") else {
      fatalError("无法找到 juliaSetCompute 函数")
    }
  ...
  func draw(commandBuffer: MTLCommandBuffer, view: MTKView) {
    ...
    computeEncoder.setBytes(&data, length: MemoryLayout<JuliaSetData>.stride, index: 0)
