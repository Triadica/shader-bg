...
class JuliaSetEffect: VisualEffect {
  var name: String = "JuliaSet"
  var displayName: String = "Julia Set"

  private var renderer: JuliaSetRenderer?
  private var commandQueue: MTLCommandQueue?
  private var lastUpdateTime: Double = 0

  func setup(device: MTLDevice, size: CGSize) {
    commandQueue = device.makeCommandQueue()
    renderer = JuliaSetRenderer(device: device, size: size)

    print("✅ JuliaSetEffect 初始化完成")
  }
