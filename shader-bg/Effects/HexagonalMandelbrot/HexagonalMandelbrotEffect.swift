...
class HexagonalMandelbrotEffect: VisualEffect {
  var name: String = "HexagonalMandelbrot"
  var displayName: String = "Hexagonal Mandelbrot"

  private var renderer: HexagonalMandelbrotRenderer?
  private var commandQueue: MTLCommandQueue?
  private var lastUpdateTime: Double = 0

  func setup(device: MTLDevice, size: CGSize) {
    commandQueue = device.makeCommandQueue()
    renderer = HexagonalMandelbrotRenderer(device: device, size: size)

    print("✅ HexagonalMandelbrotEffect 初始化完成")
  }
