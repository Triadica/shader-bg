...
class HexagonalMandelbrotRenderer {
  ...
  private func setupPipeline() {
    ...
    guard let function = library.makeFunction(name: "hexagonalMandelbrotCompute") else {
      fatalError("无法找到 hexagonalMandelbrotCompute 函数")
    }
  ...
  func update(currentTime: Double) {
    time += Float(updateInterval) * 0.5  // 0.5 是速度倍数
  }
  ...
  func draw(commandBuffer: MTLCommandBuffer, view: MTKView) {
    ...
    var data = HexagonalMandelbrotData(
      time: time,
    ...
    computeEncoder.setBytes(&data, length: MemoryLayout<HexagonalMandelbrotData>.stride, index: 0)
