//
//  SpiralForWindowsRenderer.swift
//  shader-bg
//
...
class SpiralForWindowsRenderer {
  ...
  private func setupPipeline() {
    ...
    let vertexFunction = library.makeFunction(name: "spiralForWindowsVertex")
    let fragmentFunction = library.makeFunction(name: "spiralForWindowsFragment")
  ...
  private func setupBuffers() {
    ...
    paramsBuffer = device.makeBuffer(
      length: MemoryLayout<SpiralForWindowsParams>.stride,
  ...
  func updateParticles(currentTime: CFTimeInterval) {
    ...
    // 更新时间 - 减慢到原速度的 1/4
    time += 0.00083  // 30fps * 0.00083 ≈ 每秒增加约 0.025 (原速度的 1/4)
  }
  ...
  private func updateParams(viewportSize: CGSize) {
    ...
    var params = SpiralForWindowsParams(
      time: time,
    ...
    paramsBuffer.contents().copyMemory(
      from: &params, byteCount: MemoryLayout<SpiralForWindowsParams>.stride)
  }
}
